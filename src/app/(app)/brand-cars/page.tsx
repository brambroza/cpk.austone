...
export default function BrandCarsPage() {
  ...
  async function load() {
    ...
    try {
      const data = await apiFetch<{ items: BrandRow[] }>(
        `/api/brand-cars?search=${encodeURIComponent(search)}`
      );
      setRows(data.items);
    } catch (err) {
  ...
  async function onEdit(row: BrandRow) {
    ...
    if (!nextName) return;
    try {
      await apiFetch(`/api/brand-cars/${row.id}`, {
        method: "PUT",
  ...
  async function onDelete(row: BrandRow) {
    ...
    try {
      await apiFetch(`/api/brand-cars/${row.id}`, { method: "DELETE" });
      await load();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">ยี่ห้อรถ</h1>
          <p className="text-sm text-slate-500">จัดการยี่ห้อรถในระบบ</p>
        </div>
      </div>
