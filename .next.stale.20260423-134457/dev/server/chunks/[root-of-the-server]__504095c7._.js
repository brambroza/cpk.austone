...
async function httpGetJson(url, headers) {
    const res = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$undici$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["request"])(url, {
        method: "GET",
        headers
    });
    ...
    try {
        ...
        return {
            ok: false,
            status: 500,
            text: "Invalid JSON from upstream"
        };
    }
}
...
function toDataUrlImage(buf, url) {
...
function drawKV(doc, x, y, label, value, labelW = 34) {
    doc.text(label, x, y);
...
async function generateCarReportPdf(req, params) {
    try {
        ...
        const flaskBase = process.env.FLASK_API_BASE || ("TURBOPACK compile-time value", "https://cpk.dreammaker-thai.com");
        ...
        if (!r1.ok) {
            return new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"](r1.text || "Failed to load report from Flask", {
                status: r1.status
            });
        }
        ...
        if (!report?.car) {
            return new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"]("Invalid report shape (missing car)", {
                status: 500
            });
        }
        ...
        if (!__TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].existsSync(fontPath)) {
            return new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"](`Thai font not found: ${fontPath}`, {
                status: 500
            });
        }
        const fontBase64 = __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].readFileSync(fontPath).toString("base64");
        ...
        // Title
        doc.setFontSize(14);
        doc.text("รายงานการตรวจเช็ครถ", margin, cursorY);
        cursorY += 6;
        // Header 2 คอลัมน์
        doc.setFontSize(10);
        const leftX = margin;
        ...
        drawKV(doc, leftX, cursorY + 5, "ทะเบียนรถ : ", "", 22);
        drawKV(doc, leftX, cursorY + 10, "วันที่เช็คครั้งแรก : ", "", 30);
        drawKV(doc, leftX, cursorY + 15, "เลขไมล์เริ่มต้น : ", "", 30);
        drawKV(doc, leftX, cursorY + 20, "ระยะทางใช้ไป (กม.) : ", "", 38);
        // ซ้าย value
        drawKV(doc, leftX + 18, cursorY, "", car.company_name ?? "-", 22);
        drawKV(doc, leftX + 18, cursorY + 5, "", car.registration_number ?? "-", 22);
        drawKV(doc, leftX + 18, cursorY + 10, "", summary.start_date ?? "-", 22);
        drawKV(doc, leftX + 18, cursorY + 15, "", summary.start_mile ?? "-", 22);
        drawKV(doc, leftX + 18, cursorY + 20, "", summary.distance_used ?? "-", 22);
        // ขวา
        drawKV(doc, rightX, cursorY, "สาขา : ", "", 18);
        drawKV(doc, rightX, cursorY + 5, "ประเภทรถ : ", "", 18);
        drawKV(doc, rightX, cursorY + 10, "ผู้ขับ : ", "", 18);
        drawKV(doc, rightX, cursorY + 15, "วันที่เช็คล่าสุด : ", "", 30);
        drawKV(doc, rightX, cursorY + 20, "เลขไมล์ล่าสุด : ", "", 30);
        drawKV(doc, rightX, cursorY + 25, "ครั้งที่ : ", "", 18);
        // ขวา value
        drawKV(doc, rightX + 18, cursorY, "", car.branch_name ?? "-", 18);
        drawKV(doc, rightX + 18, cursorY + 5, "", car.type ?? "-", 18);
        drawKV(doc, rightX + 18, cursorY + 10, "", car.driver ?? "-", 18);
        drawKV(doc, rightX + 18, cursorY + 15, "", summary.last_date ?? "-", 19);
        drawKV(doc, rightX + 18, cursorY + 20, "", summary.last_mile ?? "-", 18);
        drawKV(doc, rightX + 18, cursorY + 25, "", summary.round ?? "-", 18);
        cursorY += 34;
        // 5) ตาราง AutoTable
        doc.setFontSize(11);
        doc.text("รายการการเช็คยางแต่ละเส้น", margin, cursorY);
        cursorY += 2;
        const head = [
            [
                "ตำแหน่งล้อ",
        ...
        const body = tires.map((t, i)=>[
                t?.position || t?.index || `${i + 1}`,
                t?.brand_name ?? "-",
                t?.serial ?? "-",
                t?.tire_size ?? "-",
                t?.tire_identity ?? "-",
        ...
        return new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"](e?.message ?? "PDF generation failed", {
            status: 500
        });
    }
}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__504095c7._.js.map